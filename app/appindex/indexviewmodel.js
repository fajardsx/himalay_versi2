...
import { _handleAppStateChange, _reqAboutUs, Global_getreqSchedule, convertHeight, convertWidth, _saveSelectSchedule, callAlert, successLogutClear, _reqContactUs, GLOBAL_INITSCHEDULE, callToast, DebugToast, formateFullDateNumber, Global_getreqScheduleMonth, isTODAY, Global_resetShceduleSession, timeStampToDate, formatCurrentTime, getLocalSchedule, SYNCLoc_CORE_GET, SYNCLoc_CORE_SET } from '../global/Global';
...
import { SYNCLoc, TargetSelect, clean_sync_all, clean_Dokter_all, getVisitSchedule, StartExecuteFIFO } from '../appcomponent/module_async/AsyncManager';
...
const AppDrawCore = createDrawerNavigator(
  {
    HomeView: {
      screen: props => <TabController ref={refTab => { this.tab == refTab }} {...props} />
    },
  },
  {
...
class AppCore extends React.Component {
  ...
  componentDidMount() {
    ...
    if (this.props.navigation.state.params) {
      if (this.props.navigation.state.params.firstlogin) {
        this.setState({ _first: this.props.navigation.state.params.firstlogin, _email: this.props.navigation.state.params.emaillogin }, () => this.callSyncronize())
      } else {
        this.callSyncronize();
      }
    } else {
  ...
  _getData() {
    if (this.props.scheduleData == null) {
      ...
      Global_getreqSchedule(this._callback.bind(this));
    } else {
      if (this.state._first == true) {
        ...
        GLOBAL_INITSCHEDULE(this._callback.bind(this));
      }
    }
  ...
  displayWelcome() {
    ...
    return <View style={{
      ...
      <Text style={{
        ...
        {"Welcome, " + this.props.userData.profile_name + "\n\n Please Wait..."}
      </Text>
      <Text style={{
        color: "#fff",
  ...
  _callbackNewMoon(valu) {
    if (value == true) {
      ...
      callAlert(Constant.NAME_APPS, "Syncronize Data Gagal, Mohon check koneksi dan login kembali.");
      successLogutClear().then(res => {
        that.props.navigation.dispatch(
          StackActions.reset({
            index: 0,
            actions: [NavigationActions.navigate({ routeName: Constant.VM_LOGIN_SCREEN })]
          })
        )
      })
    }
  ...
  _callback(value, res = null) {
    ...
    if (value == true) {
      ...
      SYNCLoc_CORE_GET().then(res => {
        ...
        if (res) {
          ...
          if (res.attend.in != null) {
            if (user.getUserAttend().in.length == 0 && res.attend.in.length > 5 && isTODAY(res.attend.in) == true) {
          ...
          if (res.attend.out != null) {
            if (user.getUserAttend().out) {
              if (user.getUserAttend().out.length == 0 && res.attend.out.length > 5 && isTODAY(res.attend.out) == true) {
                user.updateUserAttend(res.attend);
              }
            }
          ...
          callAlert("Failed", "Syncronize Data Failed, please check your connection.and Login Again.");
        }
      });
      ...
      /*SYNCLoc(KEYS.KEY_G, KEYS.KEY_SCHEDULE).then(res => {
        ...
        if (res) {
          console.log('attend', user.getUserAttend());
          console.log('attend server', res[0].attend);
          
          if (res[0].attend.in != null) {
            if (user.getUserAttend().in.length == 0 && res[0].attend.in.length > 5 && isTODAY(res[0].attend.in) == true) {
              user.updateUserAttend(res[0].attend);
            }
          }
          console.log('update attend in', user.getUserAttend());
          if (res[0].attend.out!=null) {
            if (user.getUserAttend().out){
              if (user.getUserAttend().out.length == 0 && res[0].attend.out.length > 5 && isTODAY(res[0].attend.out) == true) {
                 user.updateUserAttend(res[0].attend);
              }
            }
          } else if (res[0].attend.out == null){
             user.updateUserAttend(res[0].attend);
          }
          ...
          console.log("out",_out);
          if(_in == 0 && _out == 0){
            let _ofin = res[0].attend.in ? res[0].attend.in.length : 0;
            let _ofout = res[0].attend.out ? res[0].attend.out.length : 0;
            //res[0].set_schedule = [];
            if (_ofin>5 && _ofout >5){
                 user.updateUserAttend(res[0].attend);
                  that.setState({
            ...
            TargetSelect().then(resSelect => {
              ...
              if (resSelect) {
                if (resSelect.length == 0){
                  ...
                  if(resSelect.length>res[0].set_schedule){
                    res[0].set_schedule = resSelect;
                  }
                }
                //_saveSelectSchedule(res[0].set_schedule)
              } else {
              ...
              SYNCLoc(KEYS.KEY_D, KEYS.KEY_SCHEDULE).then(result => {
                if (result) {
                  SYNCLoc(KEYS.KEY_A, KEYS.KEY_SCHEDULE, [res[0]]).then(() => {
                    that.setState({
                      init: true
                    })
                  });
