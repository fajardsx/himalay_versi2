...
class Cystone_page3_core extends React.Component {
  constructor(props) {
    ...
    this.xBox1 = new Animated.ValueXY({
      x: convertWidth('0%'),
      y: convertHeight('28%'),
    });
    this.xBox2 = new Animated.ValueXY({
      x: convertWidth('0%'),
      y: convertHeight('55%'),
    });
  }
  ...
  startAnim() {
    Animated.sequence([
      ...
      Animated.parallel([
        ...
        Animated.spring(this.xBox1, {
          toValue: {x: convertWidth('4%'), y: convertHeight('28%')},
          duration: 3000,
        }),
      ]),
      Animated.timing(this.fadeBook1, {
        toValue: 1,
        duration: Constant.DURATION_FADE,
      }),
      Animated.parallel([
        Animated.timing(this.fadeBox2, {toValue: 1, duration: 500}),
        Animated.spring(this.xBox2, {
          toValue: {x: convertWidth('10%'), y: convertHeight('55%')},
          duration: 3000,
        }),
      ]),
      Animated.timing(this.fadeBook2, {
        toValue: 1,
        duration: Constant.DURATION_FADE,
      }),
    ]).start();
