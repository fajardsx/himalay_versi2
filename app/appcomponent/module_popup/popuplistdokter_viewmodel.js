...
class PopListDokterViewModel extends Component{
                        ...
                        GetAllDocterThisMonth(temp).then(()=>{
                           ...
                           // console.log('data dokter ', tempData);
                            res.list_doctor_set_this_month.map(res => {
                                let found = temp.findIndex(child => { return child.id == res.id });
                                // console.log(found);
                                temp.splice(found, 1);
                            })
            ...
            /*LEMARI_GLOBAL_LOAD(KEYS.KEY_SCHEDULE).then(res => {
                if (res) {
                    ...
                    if (currentType == Constant.TYPE_MEET) {
                        let tempData = [];
                        //tempData = res[0].list_doctor_set_this_month.splice(0);
                        res[0].list_doctor_set_this_month.map(resdok=>{
                            console.log(resdok)
                            if (resdok.name){
                                tempData.push(resdok)
                            }
                    ...
                    if (currentType == Constant.TYPE_NO_MEET) {
                        ...
                        GetAllDocterThisMonth(temp).then(()=>{
                            ...
                            console.log('data dokter ', temp);
                           // console.log('data dokter ', tempData);
                            res[0].list_doctor_set_this_month.map(res => {
                                let found = temp.findIndex(child => { return child.id == res.id });
                                // console.log(found);
                                temp.splice(found, 1);
                            })
    ...
    _nameSpecalyByCategoryID(id_doctor_categories){
        ...
        let found = listCat.find(resul => { return resul.id == id_doctor_categories });
            ...
            return <View style={{
                ...
                <Text
                    style={{
                        ...
                        fontSize: convertWidth("1.2%"), color: "#000",
                        marginVertical: convertHeight('1.5%')
                    }}
                >{"List of Doctors : "}</Text>
                <FlatList
                    style={{
                    ...
                    renderItem={({ item }) =><View style={{
                        ...
                        <Text
                            style={{
                                textAlign: 'left',
                                fontFamily: Constant.POPPINS_REG,
                                fontSize: convertWidth("1.2%"), color: "#b3b3b3",
                                marginVertical: convertHeight('1.5%'),
                        ...
                        >{this._nameSpecalyByCategoryID(item.id_doctor_categories) }</Text>
                    </View>
                    }
            ...
            <View style={{
                ...
                <View style={{ alignSelf: 'center', flexDirection: 'row', marginTop: convertHeight('2%') }}>
                    ...
                    <TouchableOpacity
                        ...
                        <Text style={{ fontFamily: Constant.POPPINS_LIGHT, fontSize: convertWidth("1.5%"), color: "#fff" }}>Ok</Text>
                    </TouchableOpacity>
                </View>
...
export default PopListDokterViewModel;
