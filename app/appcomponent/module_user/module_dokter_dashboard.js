...
class DokterDashboard extends React.Component {
    ...
    onGetAddFriend() {
        if (this.props.atExpenses == null && user.getStatusRole() == Constant.ROLE_READYSTARTSCHEDULE) {
    ...
    getColorScheduleText() {
        ...
        } else if (this.props.atExpenses == null && user.getStatusRole() >= Constant.ROLE_READYSTARTSCHEDULE) {
            return Constant.COLOR_GRAY2;
        } else if (this.props.atExpenses == null && user.getStatusRole() >= Constant.ROLE_INSELECTSCHEDULE) {
            ...
            <View style={Styleapp._userdashboard}>
                ...
                <View style={Styleapp._userPhotoContainerdashboard}>
                    <Image
                        ...
                        resizeMode={'contain'}
                        source={{ uri: this.props.picture != null ? this.props.dokter.picture : "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png" }}
                    />
                </View>
                {
                    this.props.dokter != null &&
                    <View style={{ paddingRight: '5%' }}>
                        <Text style={{ fontFamily: Constant.POPPINS_MEDIUM, fontSize: convertWidth('2.3%'), color: "#000" }}>{this.props.profilDok.name != null ? this.props.profilDok.name : ""}</Text>
                        <Text style={{ fontFamily: Constant.POPPINS_ULTRALIGHT, fontSize: convertWidth('1.7%'), color: "#000" }}>{this.props.profilDok.speciality != null ? this.props.profilDok.speciality.name : ''}</Text>
                    </View>
                }

                {this.props.isDetail &&
                    <View style={{
                        position: 'absolute', width: convertWidth('25%'), height: convertHeight('10%'),
                        alignItems: 'center',
...

                        {
                            ...
                            <TouchableOpacity
                                onPress={() => this.props.onBtn1 != null ? this.props.onBtn1() : {}}
                                style={{
                                    width: convertWidth('10%'), height: convertHeight('6.5%'), justifyContent: 'center', alignItems: 'center', borderRadius: convertHeight('7%'),
                                    backgroundColor: '#ff0000'
                                }}>
                                <Text style={{ fontFamily: Constant.POPPINS_REG, fontSize: convertWidth('1.7%'), color: "#fff" }}>{"Back"}</Text>
                            </TouchableOpacity>
                        }

                        <TouchableOpacity
                            onPress={() => this.props.onBtn2 != null ? this.props.onBtn2() : {}}
                            style={{
                                marginLeft: convertWidth('3%'), width: convertWidth('18%'), height: convertHeight('6.5%'), justifyContent: 'center', alignItems: 'center', borderRadius: convertHeight('7%'),
                                backgroundColor: "#00b7a2"
                            }}>
                            <Text style={{ fontFamily: Constant.POPPINS_LIGHT, fontSize: convertWidth('1.7%'), color: "#fff" }}>{"Start"}</Text>
                        </TouchableOpacity>
                    </View>
                }
