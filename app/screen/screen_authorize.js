...
class Authorize extends Component {
  ...
  async _requestPermission() {
    ...
    const contactsStatus = await request(
    ...
    return (
      <View style={styles.container}>
        ...
        <View
          ...
          <Text style={styles.welcome}>{'W E L C O M E  T O ,'}</Text>
          <Image
        ...
        <Text style={{position: 'absolute', top: 0, left: 0, color: '#fff'}}>
          {'ver.' + Constant.NAME_VERSION}
        </Text>
        {this.props.isFirst == true && isLoading == false && (
          <TouchableOpacity
            style={styles.gettingContainer}
            onPress={() => this._onGettingStared()}>
