...
import {
    ...
    sendSchedule,
    sendAttend,
    sendAttend_SYN, sendExepense, about, contact, help, reqSchedule
} from '../module_api/module_resapi';
import haversine from 'haversine';
import user from './user'
import { LEMARI_GLOBAL_LOAD, LEMARI_GLOBAL_LOC, ASYNC_getScheduleToday, TargetSelect, getIDSAsync, LEMARI_GLOBAL_SELECT, SYNCLoc } from '../appcomponent/module_async/AsyncManager';
...
export function getMonthSchedulerDoktor(data) {
    ...
    if(data !=undefined && data!=null && data.length>0){
        data.map(hos => {
            hos.doctors.map(dok => {
                if (countDoktor.find(res => { res.id == dok.id }) == undefined) {
...
export async function GetAllDocterCound(callback) {
    await DATA_SCHEDULE.getDataSchedule().then(res => {
    ...
    return await DATA_SCHEDULE.getDataSchedule().then(res => {
        ...
        listSelect.map(selectId => {
            resus.visit_schedule.map(resHospital => {
                let dataDokter = resHospital.doctors.find(dok => dok.pivot.doctors_id = selectId);
                if (dataDokter) {
                    //console.log(dataDokter);
                    if (tempDokter.findIndex(savedoc => savedoc.pivot.doctors_id = dataDokter.pivot.doctors_id) == -1) {
